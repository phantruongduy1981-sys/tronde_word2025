...
def check_is_marked_answer(block):
    ...
    for r in runs:
        ...
        # Check Bold (Optional - Uncomment if needed)
...
def clean_short_answer_content(block):
    ...
    if match:
        ...
        # XÓA TEXT trong XML: Duyệt từng node text, nếu chứa từ khóa thì xóa
        for r in runs:
            t_nodes = r.getElementsByTagNameNS(W_NS, "t")
            for t in t_nodes:
                if t.firstChild and t.firstChild.nodeValue:
                    val = t.firstChild.nodeValue
                    # Nếu node này chứa chữ ĐS...
                    if re.search(r'(ĐS|Đáp số|DS)', val, re.IGNORECASE):
                        # Giữ lại phần trước đó (nếu có), xóa phần sau
                        m_part = re.search(r'(.*?)(ĐS|Đáp số|DS)', val, re.IGNORECASE)
                        if m_part:
                            t.firstChild.nodeValue = m_part.group(1)
                            # Đặt thuộc tính space để tránh dính chữ
                            t.setAttribute("xml:space", "preserve") 
                    
                    # Nếu đây là node chứa giá trị đáp án (sau khi đã tìm thấy ĐS), xóa luôn
                    # (Logic đơn giản hóa: nếu đã extract được answer, ta clear các node text phía sau ĐS)
                    # Tuy nhiên để an toàn, hàm này chủ yếu xóa định dạng. 
                    # Việc xóa triệt để text rác có thể phức tạp nếu nó nằm rải rác nhiều run.
                    pass
    return found_answer
...
def update_label_smart(paragraph, regex_pattern, new_label_text):
    ...
    Thay vì replace toàn bộ text, chỉ tìm đúng node chứa label và thay thế phần label đó.
    Giữ nguyên các node text khác và thuộc tính xml:space.
    """
    t_nodes = paragraph.getElementsByTagNameNS(W_NS, "t")
    ...
    for i, t in enumerate(t_nodes):
        ...
        # Kiểm tra xem node này có chứa pattern không (VD: "Câu 1." hoặc "A.")
        m = re.match(regex_pattern, txt, re.IGNORECASE)
        if m:
            # Tìm thấy! Thay thế phần match bằng label mới
            # Giữ lại phần text thừa phía sau (nếu có)
            ...
            # QUAN TRỌNG: Đảm bảo thuộc tính space để không bị dính chữ
            t.setAttribute("xml:space", "preserve")
            
            # Tô màu cho run chứa node này
            run = t.parentNode
            if run and run.localName == "r": 
                style_run_blue_bold(run)
            
            # Xóa các node rác ngay sau đó nếu nó chỉ chứa dấu chấm/ngoặc thừa (do Word tách run)
            # VD: Run1="A", Run2="." -> Sau khi Run1 sửa thành "B.", Run2 thừa "."
            for j in range(i + 1, len(t_nodes)):
                t2 = t_nodes[j]
                if t2.firstChild and t2.firstChild.nodeValue:
                    # Nếu node tiếp theo chỉ là dấu chấm/ngoặc và khoảng trắng
                    if re.match(r'^[\.\)\s]+$', t2.firstChild.nodeValue):
                        t2.firstChild.nodeValue = "" 
                    else:
                        break # Gặp chữ khác thì dừng
            
            found = True
            break # Chỉ sửa label đầu tiên tìm thấy
...
def process_mcq_question(question_blocks):
    ...
    # Đánh lại nhãn (A. B. C. D.) - Dùng hàm SMART
    for idx, block in enumerate(shuffled_blocks):
        lbl = chars[idx] if idx < 4 else chars[-1]
        # Regex: Bắt đầu bằng A/B/C/D, có thể có dấu . hoặc )
...
def process_full_docx(file_bytes, code_name):
    ...
    def handle_part(start_idx, end_idx, part_type):
        ...
        for idx, (q_blks, ans) in enumerate(shuffled_qs_with_ans):
            if q_blks:
                # Update "Câu X." -> Dùng hàm SMART
                update_label_smart(q_blks[0], r'^(\s*)(Câu\s*)(\d+)(\.)?', f"Câu {idx+1}.")
...
def generate_answer_key_doc(all_exam_data):
    ...
    # FIX LỖI THIẾU CỘT: Tìm max số câu hỏi trong TẤT CẢ các mã đề
...
def main():
    ...
    with col1:
        with st.expander("📄 Hướng dẫn & File Mẫu", expanded=True):
            st.info("""
            **Lưu ý định dạng:**
            1. Hệ thống đã cập nhật để **Giữ nguyên định dạng** (khoảng cách chữ, công thức toán).
            2. Đáp án: Gạch chân, Tô màu hoặc Highlight.
            """)
            file_mau_url = "https://docs.google.com/document/d/1i1b-By6EA_HO8fWgMYG9iXZPGannmWdg/export?format=docx"
            st.link_button("📥 Tải File Word Mẫu", file_mau_url, help="Tải file mẫu chuẩn", type="secondary")
